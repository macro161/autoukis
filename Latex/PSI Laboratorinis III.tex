...
\section{Misija ir vizija}
\begin{itemize}
\item{Misiją - sutekti žmonėms pigesnį ir kokybiškesnį žemės ūkio produktą.}
\item {Vizija - pasaulis kuriame fiziniį darbą už žmogų atlieka technologijos.}
\end{itemize}
...
\item Sistemos vartotojas gali susikurti naują vartotojo paskyrą
\begin{figure}[H]
		\centering	
	\includegraphics[width=18cm,height=20cm,keepaspectratio]{Registracija.jpg}
	\caption{Registracijos sekų diagrama}
	\label{fig:registracija}
	\end{figure}
	Tam, kad vartotojas galėtų naudotis sistema, jis pirma turi susikurti naują vartotojo paskyrą.
\item Sistemos vartotojas gali pakeisti laistymo sistemų parametrus
\begin{figure}[H]
		\centering	
	\includegraphics[width=18cm,height=20cm,keepaspectratio]{Laistymas.jpg}
	\caption{Laistymo parametrų pakeitimo sekų diagrama}
	\label{fig:laistymas}
	\end{figure}
\end{itemize}
	Sistemos varototojas gali pakeisti laistymo sistemų parametrus (laistymo intensyvumą, periodiškumą ir pan.).
\item Sistemos vartotojas gali keisti žemėlapio žymėjimus
\begin{figure}[H]
		\centering	
	\includegraphics[width=18cm,height=20cm,keepaspectratio]{zemelapis.jpg}
	\caption{Žemėlapio žymėjimų pakeitimo sekų diagrama}
	\label{fig:zemelapis}
	\end{figure}
	Sistemos vartotojas gali peržiūrėti žemėlapio žymėjimus bei juos modifikuoti arba ištrinti.
\end{itemize}
...
Yra keletas teisinių aspektų į kuriuos vertėtų atsižvelgti siekiant įgyvendinti verslo idėja ir sistemą. 
	
\begin{itemize}
		\item{Darbo kodeksas: Kadangi mūsų įmonės samdys darbuotojus, svaru nepamiršti laikytis darbo kodekso. Darbuotojų algos, atostogos ir panašūs klausimai turi būti aiškiai apibrėžti darbo sutartyje. Svarbu ir darbuotojo atleidimo aspektas, kad abi pusės žimotų, kokiom aplinkybėm esant darbuotojas gali reikalauti kompensacijų ar kada įmonė atleis darbuotoją}
\item{LR veterinarijos įstatymai: sistema neatsako už gyvūnų sveikatą, neteisingai naudojama sistema gali sukelti pavojų gyvūnų gyvybei tačiau kūrėjai už tai neatsako}
\item{Informacijos saugumas: sismetos kūrėjai įsipareigoja vartotojų informaciją laikyti pagal naujausius informacijos saugumo reikalavimus. Kūrėjas taip pat įsipareigoja darbuotojų darbo kontrakte pateiki konfidencialumo sutartį, pagal kurią nebus viešinama jokia informacija.}
		\item{Kūrėjai įsipareigoja laikytis Lietuvos įstatymų ir konstitucijos}
